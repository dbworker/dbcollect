...
# SunOS's change:
#     sh        => bash      // export xx=xx; if [ == ]
#     awk -v    => nawk -v
#     tr 'a-z'  => tr [a-z]
#     expr length => NONE, only support in x86
#     awk {BS...} => fail
#
...
;;
SunOS)
    sh $mToolPath/script/dump_host_sunos.sh "$mVersion"
    AWK=nawk
;;
*)
